...
def do_fc_inputs(method, out, config):
    ...
    if method_version is None:
        payload = cumulus_util.get_latest_method(method_namespace, method_name)['payload']
        ...
        if config is not None:
            config_namespace, config_name, config_version = cumulus_util.fs_split(config)
        ...
        jar = pkg_resources.resource_filename('cumulus_util', 'womtool-30.2.jar')
