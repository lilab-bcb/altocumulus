...
def main(argv):
    parser = argparse.ArgumentParser(
        description='Query project metadata from a LIMS (Laboratory Information Management System) via RESTful APIs.\n\n\
...
Users who want to use this subcommand need to write their own lims_query package, which should at least contain one function:\n\
    query_ngs(project_id: str) -> pd.DataFrame.',
    ...
    parser.add_argument(
        ...
        choices=["ngs", "project"],
        default="ngs",
        help='Specify query type. Choose from "ngs" for FASTQ info or "project" for project metadata.',
    )
    ...
    try:
        from lims_query import query_ngs, query_project
