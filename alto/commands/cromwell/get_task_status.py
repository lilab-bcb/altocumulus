...
class JobIDFetcher:
    def __init__(self, server, port):
        ...
        self.workflow_jobs = {}
    def get_metadata(self, job_id):        # requests and returns the metadata
        resp = requests.get(self.base_url + f"{job_id}/metadata")
        d = resp.json()
        return d['calls']
    def get_workflow_status(self, job_id):     # returns the final dictionary with all jobIDs
    ...
    def get_jobID(self, task_name, task_metadata):  # returns a dictionary with IDs and status for each task
        jobID_status = {}
        task_status = {}
        ...
        return task_status
    def subworkflow_IDs(self, task_metadata):   # since dict is created for each subworkflow tasks, they are combined to a single dict before returning
        subworkflow_status = {}
