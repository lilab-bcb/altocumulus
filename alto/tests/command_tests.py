...
class CommandTests(unittest.TestCase):
    ...
    def test_upload_sample_sheet(self):
        alto.commands.sample_sheet.main(['-d', 'inputs', '-o', 'test_sample_sheet.txt'])
        alto.commands.fc_upload.main(['--dry-run', '-w', 'cumulus-dev/test', 'test_sample_sheet.txt'])
        os.remove('test_sample_sheet.txt')

    def test_run(self):
        alto.commands.fc_run.main(['-m', 'broadgdac/echo', '-i', 'test.json', '-w', 'cumulus-dev/test'])

    def test_inputs(self):
        alto.commands.fc_inputs.main(['-m', 'broadgdac/echo', '-o', 'test-inputs.json'])
        os.remove('test.json')

    def test_add_method(self):
        alto.commands.fc_add_method.main(['-n', 'cumulus-dev', 'inputs/echo.wdl'])

    def test_remove_methods(self):
        alto.commands.fc_remove_method.main(['-m', 'cumulus-dev/'])
