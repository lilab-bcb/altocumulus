...
def do_fc_inputs(method, out, config):
    ...
    if method_version is None:
        payload = sccutil.get_latest_method(method_namespace, method_name)['payload']
        ...
        if config is not None:
            config_namespace, config_name, config_version = sccutil.fs_split(config)
        ...
        jar = pkg_resources.resource_filename('sccutil', 'womtool-30.2.jar')
